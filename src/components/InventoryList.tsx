...
function expiryStyle(days: number | null) {
  if (days === null) return { chip: "bg-black/5 text-muted", label: "No date", accent: "border-l-transparent" };
  if (days <= 0) return { chip: "bg-coral/15 text-coral", label: "Expired", accent: "border-l-coral" };
  if (days <= 2) return { chip: "bg-coral/15 text-coral", label: `${days}d left`, accent: "border-l-coral" };
  if (days <= 5) return { chip: "bg-amber/25 text-[#a76a14]", label: `${days}d left`, accent: "border-l-amber" };
