...
const ListIcon = () => (
  <Svg>
    ...
    <path d="M8.5 12H20" />
    <path d="M8.5 17.5H20" />
    <path d="m3.3 6 .9.9 1.8-2" />
    <path d="m3.3 11.5.9.9 1.8-2" />
    <path d="m3.3 17 .9.9 1.8-2" />
  </Svg>
);
const CameraIcon = () => (
  <Svg>
...
const items = [
  { href: "/planner", label: "Meal Plan", icon: <CalendarIcon /> },
  { href: "/shopping", label: "Shopping", icon: <ListIcon /> },
  { href: "/scan", label: "Scan", fab: true },
  { href: "/inventory", label: "Pantry", icon: <FridgeIcon /> },
  { href: "/recipes", label: "Recipe", icon: <BookIcon /> },
];
...
export default function Nav() {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 z-20 w-full max-w-[440px] -translate-x-1/2 border-t border-black/[0.06] bg-cream/90 backdrop-blur-md">
      <ul className="flex items-end justify-around px-1 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
        {items.map((it) => {
          ...
          return (
            <li key={it.href} className="flex-1">
              <Link
                href={it.href}
                className={`flex flex-col items-center gap-1 py-1 text-center text-[10px] font-semibold leading-tight transition-colors ${
